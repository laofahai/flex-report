...
export function ExcelDesigner() {
  ...
  const [tableDesign, setTableDesign] = useState<TableDesign>({
    ...
    schema: {
      rows: [
        {
          type: 'normal',
          cells: Array.from({ length: initialColumnsLength }, () => ({
            value: '',
          })) as ExcelDesignerCell[],
        },
        {
          type: 'loop',
          cells: Array.from({ length: initialColumnsLength }, () => ({
      ...
      columns: Array.from({ length: initialColumnsLength }, (_, idx) => ({
  ...
  function handleGoPreview() {
    ...
    window.open(`/excel/${designId}/preview`, '_blank')
  }
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 text-xs min-w-32">
              <SelectValue
                placeholder={dataSources.length === 0 ? '无可用数据源' : '请选择数据源'}
              />
            </SelectTrigger>
            <SelectContent>
          ...
          <Button className="ml-2" onClick={handleGoPreview} variant={'secondary'}>
            预览
          </Button>
            ...
            onDataChange={(newData: ExcelDesignerRow[]) => {
              ...
              setTableDesign((prev) => ({
                ...
                schema: {
                  ...prev.schema,
                  rows: fixedRows,
                  columns: prev.schema.columns?.length
                    ? prev.schema.columns
                    : Array.from({ length: fixedRows[0].cells.length }, (_, idx) => ({
                        width: initialColumnWidth,
                      })),
                },
              }))
            ...
            onColumnWidthChange={(colIdx, width) => {
              setTableDesign((prev) => {
                if (!prev.schema.columns?.length) {
                  prev.schema.columns = Array.from(
                    { length: prev.schema.rows[0].cells.length },
                    () => ({ width: initialColumnWidth })
                  )
                }
